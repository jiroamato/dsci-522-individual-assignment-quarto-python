...
![Horse populations for all provinces in Canada from 1906 - 1972](../results/horse_pops_plot.png ){#fig-horse-plot-1 width=65%}

We can see from @fig-horse-plot-1 that Ontario, Saskatchewan and Alberta 
...
Additionally, note that in @tbl-sd we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-plot-1.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-horse-plot-2 width=65%}

In @fig-horse-plot-2, we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
